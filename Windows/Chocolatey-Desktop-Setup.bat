...
REM choco install -y PowerISO is a powerful CD / DVD / BD image file processing tool, which allows you to open, extract, burn, create, edit, compress, encrypt, split and convert ISO files, and mount ISO files with internal virtual drive.
...
REM choco install -y osh-github REM PowerShell cmdlet helpers for working with the GitHub API. * New-GitHubOAuthToken - creates an OAuth token for use with GitHub
...
REM choco install -y office365-2016-deployment-tool REM v
...
REM choco install -y powershell REM latest version of windoiws management framewiork
...
REM choco install -y keyhac REM custimize key input
...
REM choco install -y adfender REM bloack all ads, especially youtube
